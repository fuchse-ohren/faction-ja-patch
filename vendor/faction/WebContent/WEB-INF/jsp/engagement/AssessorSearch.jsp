...
<bs:mco colsize="6">
 ...
 <bs:row>
  ...
  <s:if test="currentAssessment.InPr"> <span class="text-warning fa fa-eye"></span><b class="text-warning"> (査読中)</b></s:if>
  <s:if test="currentAssessment.prComplete"> <span class="text-success fa fa-eye"></span><b class="text-success"> (査読完了)</b></s:if>
  <s:if test="currentAssessment.Finalized"> <span class="text-primary fa fa-check"></span><b  class="text-primary"> 検査完了</b></s:if>
  </span>
  </h3>
	  <hr>
	  </bs:mco>
	...
	<bs:row>
		 ...
		 <bs:inputgroup name="検査対象名: <b>*</b>" id="appName" colsize="4" ><s:property value="currentAssessment.name" /></bs:inputgroup>
		 <bs:dt name="期間: <b>*</b>" colsize="4" id="reservation" readOnly="${currentAssessment.isFinalized() }"><s:property value="startStr"/> to <s:property value="endStr"/></bs:dt>
		 <bs:select name="Select 担当組織: <b>*</b>" colsize="4" id="teamName" readOnly="${currentAssessment.isFinalized() }">
		 	<s:iterator value="teams">
	            <option value="<s:property value="id"/>"><s:property value="teamName"/></option>
	        </s:iterator>
		 </bs:select>
		 <bs:select name="検査種別: <b>*</b>" colsize="4" id="assType"  readOnly="${currentAssessment.isFinalized() }">
			<s:iterator value="assessmentTypes">
	            <option value="<s:property value="id"/>"><s:property value="type"/></option>
	        </s:iterator>
		 </bs:select>
		 <bs:select name="キャンペーン: <b></b>" colsize="4" id="campName" >
		...
		<bs:select name="状態:" colsize="4" id="statusSelect">
	 ...
	 <bs:row>
	 	...
	 	<bs:inputgroup colsize="12" name="配布:" id="distlist"  readOnly="${currentAssessment.isFinalized() }"><s:property value="currentAssessment.DistributionList"/> </bs:inputgroup>
	 </bs:row>
	 <br>
